...
public abstract class Command extends org.powernukkitx.command.Command implements PluginIdentifiableCommand {
	...
	@Override public final synchronized Plugin getPlugin() {
		if (this.plugin == null) {
	...
	@Override public final void enableCommandTree() {
	}

	@Override public final void enableParamTree() {
	}
	...
	public final synchronized void initialize() {
		...
		if (this.initializing) {
			throw new CommandConfigurationException("Circular initialization detected for command '" + this.getName() + "'");
		}
	...
	public final Command addRule(Rule rule) {
	...
	public final String getCommandPermission() {
	...
	public final List<String> getUsageLines() {
		this.initialize();
	...
	public final CommandResult dispatch(@NonNull CommandSender sender, @NonNull String commandLabel, @NonNull String[] arguments) {
		...
		try {
			for (CommandNodeData node : path) {
				...
				if (!ruleResult.isSuccess()) {
					return new CommandFailure(sender, CommandFailureReason.RULE_FAILED, CommandMessages.get(CommandMessages.COMMAND_RULE_FAILED), usage, Map.of(), ruleResult.failed(), null);
				}
			}
			...
			CommandContext context = new CommandContext(sender, parsedArguments, label.toString(), nodeArguments, this, target);
			Object returnValue = target == null ? this.onRun(context) : target.run(context);
			...
			return new CommandFailure(sender, exception.getReason(), exception.getMessage(), usage, exception.getDetails(), List.of(), exception);
		} catch (Throwable throwable) {
			return new CommandFailure(sender, CommandFailureReason.EXECUTION_ERROR, CommandMessages.get(CommandMessages.COMMAND_EXECUTION_ERROR), usage, Map.of(), List.of(), throwable);
		}
	}

	@Override public final boolean execute(CommandSender sender, String commandLabel, String[] args) {
	...
	protected void fail(CommandFailure result) {
		...
		if (result.usage() != null && result.reason() != CommandFailureReason.EXECUTION_ERROR) {
			CommandMessages.send(result.sender(), CommandMessages.COMMAND_USAGE, Map.of("usage", result.usage()));
		}
		if (result.cause() != null && result.reason() == CommandFailureReason.EXECUTION_ERROR) {
			this.getPlugin().getLogger().error("Command /" + this.getName() + " failed", result.cause());
		}
	}
	...
	public final synchronized void refreshDefinition() {
		if (!this.initialized) {
			return;
		}
	...
	private void resolvePermission() {
		...
		this.commandPermission = nativePermission == null || nativePermission.isBlank() ? this.getName().toLowerCase(Locale.ROOT) + ".command" : nativePermission;

		this.registerPermission(this.commandPermission, this.getDescription(), Permission.DEFAULT_OP);
	...
	private void applyNativeMetadata() {
		...
		if (name == null || name.isBlank()) {
			throw new CommandConfigurationException("Command " + this.getClass().getName() + " needs PNX @CommandDefinition or an explicit name constructor");
		}
	...
	private void addPermissionRule() {
		boolean hasRule = this.getRules().stream().filter(PermissionRule.class::isInstance).map(PermissionRule.class::cast).anyMatch(rule -> rule.getPermission().equals(this.commandPermission));
		if (!hasRule) {
	...
	private void addOverloads(Map<String, CommandParameter[]> overloads, String key, List<CommandParameter> prefix, CommandNodeData node, @Nullable CommandSender viewer) {
		...
		for (SubCommand child : node.getSubCommands()) {
			...
			for (int index = 0; index < labels.size(); index++) {
				...
				childPrefix.add(CommandParameter.newEnum(label, false, new String[]{label}));

				String childKey = key + "_" + child.getName();
				if (index > 0) {
	...
	private static Plugin resolveOwningPlugin(Class<?> commandType) {
		...
		if (server == null) {
			throw new CommandConfigurationException("Cannot resolve the owning plugin for command " + commandType.getName() + " before the server is initialized");
		}

		ClassLoader commandLoader = commandType.getClassLoader();
		List<Plugin> matches = server.getPluginManager().getPlugins().values().stream().filter(plugin -> plugin.getClass().getClassLoader() == commandLoader).toList();

		if (matches.size() != 1) {
			throw new CommandConfigurationException("Unable to resolve exactly one owning plugin for command " + commandType.getName() + "; found " + matches.size());
		}
