...
final public class FloatArgument extends Argument<Double> {
    ...
    private FloatArgument(String name, boolean optional, boolean hasDefault, Double defaultValue, double minimum, double maximum) {
        ...
        if (!Double.isFinite(minimum) || !Double.isFinite(maximum) || minimum > maximum) {
            throw new CommandConfigurationException(
                "Invalid numeric range for argument '" + name + "'"
            );
        }
    ...
    @Override protected Double parseValue(CommandSender sender, String value) {
        ...
        try {
            ...
            throw new ArgumentParseException(
                this.getName(),
                value,
                CommandMessages.format(CommandMessages.ARGUMENT_NUMBER, "argument", this.getName()),
                exception
            );
        }
        ...
        if (!Double.isFinite(parsed) || parsed < this.minimum || parsed > this.maximum) {
            throw new ArgumentParseException(
                ...
                CommandMessages.format(
                    CommandMessages.ARGUMENT_NUMBER_RANGE,
                    Map.of(
