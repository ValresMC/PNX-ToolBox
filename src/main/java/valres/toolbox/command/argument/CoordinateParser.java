    ...
    public static Vector3 parse(CommandSender sender, String argumentName, String value) {
        ...
        if (coordinates.length != 3) {
            throw new ArgumentParseException(
                argumentName,
                value,
                CommandMessages.format(CommandMessages.ARGUMENT_COORDINATES_COUNT, "argument", argumentName)
            );
        }
        ...
        return new Vector3(
            parseCoordinate(sender, argumentName, coordinates[0], origin.getX()),
            parseCoordinate(sender, argumentName, coordinates[1], origin.getY()),
            parseCoordinate(sender, argumentName, coordinates[2], origin.getZ())
        );
    }
    ...
    private static double parseCoordinate(CommandSender sender, String argumentName, String coordinate, double origin) {
        ...
        if (relative && !sender.isEntity()) {
            throw new ArgumentParseException(
                argumentName,
                coordinate,
                CommandMessages.get(CommandMessages.ARGUMENT_RELATIVE_COORDINATES)
            );
        }
        ...
        if (!Double.isFinite(parsed)) {
            throw new ArgumentParseException(
                argumentName,
                coordinate,
                CommandMessages.format(CommandMessages.ARGUMENT_COORDINATE_RANGE, "coordinate", coordinate)
            );
        }
    ...
    private static double getOffset(String argumentName, String coordinate, boolean relative) {
        if (coordinate.startsWith("^")) {
            throw new ArgumentParseException(
                argumentName,
                coordinate,
                CommandMessages.get(CommandMessages.ARGUMENT_LOCAL_COORDINATES)
            );
        }
        ...
        if (!numericPart.isEmpty()) {
            try {
                ...
                throw new ArgumentParseException(
                    argumentName,
                    coordinate,
                    CommandMessages.format(CommandMessages.ARGUMENT_INVALID_COORDINATE, "coordinate", coordinate),
                    exception
                );
            }
        } else if (!relative) {
