...
public class OptionsArgument<T> extends Argument<T> {
	private final Map<@NonNull String, @NonNull T> values;
	...
	protected OptionsArgument(String name, @NonNull Map<@NonNull String, @NonNull T> values, boolean optional, boolean hasDefault, T defaultValue) {
		...
		for (Map.Entry<@NonNull String, @NonNull T> entry : values.entrySet()) {
			...
			if (option.isEmpty() || option.chars().anyMatch(Character::isWhitespace)) {
				throw new CommandConfigurationException("Option names for argument '" + name + "' cannot be empty or contain spaces");
			}

			String key = option.toLowerCase(Locale.ROOT);
			if (normalized.containsKey(key)) {
				throw new CommandConfigurationException("Duplicate option '" + option + "' for argument '" + name + "'");
			}
			normalized.put(key, Objects.requireNonNull(entry.getValue(), "Option value cannot be null"));
		}
		if (normalized.isEmpty()) {
			throw new CommandConfigurationException("Option argument '" + name + "' must contain at least one value");
		}
		if (hasDefault && !normalized.containsValue(defaultValue)) {
			throw new CommandConfigurationException("Default value is not present in options for argument '" + name + "'");
		}
	...
	@Override protected T parseValue(CommandSender sender, String value) {
		...
		if (!this.values.containsKey(key)) {
			throw new ArgumentParseException(this.getName(), value, CommandMessages.format(CommandMessages.ARGUMENT_OPTION, Map.of("argument", this.getName(), "options", String.join(", ", this.names))));
		}
