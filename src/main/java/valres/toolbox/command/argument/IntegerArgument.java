...
public final class IntegerArgument extends Argument<Integer> {
	...
	private IntegerArgument(String name, boolean optional, boolean hasDefault, Integer defaultValue, int minimum, int maximum) {
		...
		if (minimum > maximum) {
			throw new CommandConfigurationException("Minimum cannot exceed maximum for argument '" + name + "'");
		}
	...
	@Override protected Integer parseValue(CommandSender sender, String value) {
		...
		try {
			...
			throw new ArgumentParseException(this.getName(), value, CommandMessages.format(CommandMessages.ARGUMENT_INTEGER, "argument", this.getName()), exception);
		}

		if (parsed < this.minimum || parsed > this.maximum) {
			throw new ArgumentParseException(this.getName(), value, CommandMessages.format(CommandMessages.ARGUMENT_INTEGER_RANGE, Map.of("argument", this.getName(), "minimum", this.minimum, "maximum", this.maximum)));
		}
