...
public final class LevelArgument extends Argument<Level> {
	...
	@Override protected Level parseValue(CommandSender sender, String value) {
		...
		if (level == null) {
			throw new ArgumentParseException(this.getName(), value, CommandMessages.format(CommandMessages.ARGUMENT_LEVEL_NOT_LOADED, "level", value));
		}
	...
	private Collection<String> getLevelNames() {
		return Server.getInstance().getLevels().values().stream().map(Level::getName).distinct().sorted(String.CASE_INSENSITIVE_ORDER).toList();
	}
}
