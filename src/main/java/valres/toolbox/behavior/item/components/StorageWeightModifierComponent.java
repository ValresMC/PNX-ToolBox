...
final public class StorageWeightModifierComponent extends DataDrivenItemComponent {
    final private int weightInStorageItem;

    public StorageWeightModifierComponent(int weightInStorageItem) {
        if (weightInStorageItem < 0 || weightInStorageItem > 64) {
            throw new IllegalArgumentException(
                "Component 'minecraft:storage_weight_modifier', value 'weight_in_storage_item' "
                    + "must be between 0 and 64, got " + weightInStorageItem
            );
        }
