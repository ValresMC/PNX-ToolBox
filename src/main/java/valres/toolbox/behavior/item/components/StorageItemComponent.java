...
final public class StorageItemComponent extends DataDrivenItemComponent {
    final private int maxSlots;
    final private Boolean allowNestedStorageItems;
    final private List<?> allowedItems;
    final private List<?> bannedItems;

    public StorageItemComponent(int maxSlots) {
        this(maxSlots, null, null, null);
    }

    public StorageItemComponent(
        int maxSlots,
        Boolean allowNestedStorageItems,
        List<?> allowedItems,
        List<?> bannedItems
    ) {
        if (maxSlots < 1 || maxSlots > 64) {
            throw new IllegalArgumentException(
                "Component 'minecraft:storage_item', value 'max_slots' "
                    + "must be between 1 and 64, got " + maxSlots
            );
        }
