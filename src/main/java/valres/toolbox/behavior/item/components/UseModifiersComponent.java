...
final public class UseModifiersComponent extends DataDrivenItemComponent {
    final private float useDuration;
    final private Boolean emitVibrations;
    final private Float movementModifier;
    final private String startSound;

    public UseModifiersComponent(float useDuration) {
        this(useDuration, null, null, null);
    }

    public UseModifiersComponent(
        float useDuration,
        Boolean emitVibrations,
        Float movementModifier,
        String startSound
    ) {
        if (movementModifier != null && (movementModifier < 0 || movementModifier > 1)) {
            throw new IllegalArgumentException(
                "Component 'minecraft:use_modifiers', value 'movement_modifier' "
                    + "must be between 0.0 and 1.0, got " + movementModifier
            );
        }
