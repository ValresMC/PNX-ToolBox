...
final public class RecordComponent extends DataDrivenItemComponent {
    final private int comparatorSignal;
    final private float duration;
    final private String soundEvent;
    ...
    public RecordComponent(int comparatorSignal, float duration, @NonNull String soundEvent) {
        if (comparatorSignal < 1 || comparatorSignal > 13) {
            throw new IllegalArgumentException(
                "Component 'minecraft:record', value 'comparator_signal' "
                    + "must be between 1 and 13, got " + comparatorSignal
            );
        }
