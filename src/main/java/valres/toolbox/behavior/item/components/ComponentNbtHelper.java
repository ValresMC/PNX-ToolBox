...
public final class ComponentNbtHelper {
	...
	public static @NonNull CompoundTag compound(@NonNull Map<?, ?> values) {
		...
		for (Map.Entry<?, ?> entry : values.entrySet()) {
			if (entry.getKey() == null) {
				throw new IllegalArgumentException("An NBT compound key cannot be null");
			}
	...
	public static @NonNull CompoundTag compound(Object... keyValuePairs) {
		if (keyValuePairs.length % 2 != 0) {
			throw new IllegalArgumentException("Compound values must be provided as key/value pairs");
		}
	...
	public static @NonNull ListTag<StringTag> stringList(@NonNull Collection<String> values) {
		...
		for (String value : values) {
			if (value == null) {
				throw new IllegalArgumentException("An NBT string list cannot contain null");
			}
	...
	public static @NonNull ListTag<CompoundTag> compoundList(@NonNull Collection<?> values) {
		...
		for (Object entry : values) {
			...
			if (value instanceof CompoundTag compound) {
				...
				throw new IllegalArgumentException("A compound list only accepts Map, CompoundTag or ItemComponentValue entries");
			}
		}
	...
	public static @NonNull Tag tag(Object value) {
		...
		switch (value) {
			case null -> throw new IllegalArgumentException("NBT has no null tag; put null in a compound to omit the entry");
	...
	private static @NonNull Tag collectionTag(@NonNull Collection<?> values) {
		List<Object> normalized = values.stream().map(ComponentNbtHelper::normalizeValue).toList();
		...
		throw new IllegalArgumentException("NBT lists must contain values of one compatible type");
	}
	...
	private static @NonNull ListTag<Tag> tagList(@NonNull Collection<Tag> values) {
		...
		for (Tag value : values) {
			if (value.getId() != tagId) {
				throw new IllegalArgumentException("An NBT tag list cannot mix " + Tag.getTagName(tagId) + " and " + Tag.getTagName(value.getId()));
			}
			result.add(value);
		}
	...
	private static @NonNull NumericListType numericListType(@NonNull Collection<? extends Number> values) {
		...
		for (Number value : values) {
			if (value == null) {
				throw new IllegalArgumentException("An NBT numeric list cannot contain null");
			}
			if (value instanceof Double) {
				...
				throw new IllegalArgumentException("Unsupported NBT number type: " + value.getClass().getName());
			}
		}
