...
public final class CustomBlockRegistry {
	private static final CustomBlockRegistry INSTANCE = new CustomBlockRegistry();

	private final Map<String, RegisteredBlockData> blocks = new LinkedHashMap<>();
	...
	public synchronized @NonNull BlockBuilder register(@NonNull Class<? extends Block> blockClass) {
		Block block = this.createBlock(blockClass);
		return this.register(BlockBuilder.create(block));
	}

	public synchronized @NonNull BlockBuilder register(@NonNull Class<? extends Block> blockClass, @NonNull Class<? extends Item> blockItemClass) {
		Block block = this.createBlock(blockClass);
		return this.register(BlockBuilder.create(block), blockItemClass);
	}

	public synchronized @NonNull BlockBuilder register(@NonNull BlockBuilder builder) {
		return this.registerInternal(builder, null);
	}

	public synchronized @NonNull BlockBuilder register(@NonNull BlockBuilder builder, @NonNull Class<? extends Item> blockItemClass) {
		return this.registerInternal(builder, blockItemClass);
	}

	private @NonNull BlockBuilder registerInternal(@NonNull BlockBuilder builder, @Nullable Class<? extends Item> blockItemClass) {
		...
		if (!(block instanceof CustomBlock)) {
			throw new IllegalArgumentException("Block class must implement CustomBlock or ToolBoxBlock");
		}
		...
		try {
			PNXBlockRegistryAccessor.register(block, block.getClass().asSubclass(Block.class), definition, !hasLinkedItem);
		} catch (RegisterException exception) {
			throw new IllegalStateException("Cannot register block '" + identifier + "'", exception);
		}
		...
		this.blocks.put(identifier, new RegisteredBlockData(block, block.getClass().asSubclass(Block.class), builder, definition));
	...
	private @NonNull Block createBlock(@NonNull Class<? extends Block> blockClass) {
		try {
			Constructor<? extends Block> constructor = blockClass.getConstructor(BlockState.class);
			return constructor.newInstance((BlockState) null);
		} catch (ReflectiveOperationException exception) {
			throw new IllegalArgumentException("Block class must have a public constructor accepting BlockState", exception);
		}
	}

	private void validateBlockClass(@NonNull Block block) {
		Class<? extends Block> blockClass = block.getClass().asSubclass(Block.class);
		if (Modifier.isAbstract(blockClass.getModifiers())) {
			throw new IllegalArgumentException("Block class cannot be abstract");
		}

		try {
			Field propertiesField = blockClass.getDeclaredField("PROPERTIES");
			int modifiers = propertiesField.getModifiers();
			boolean validField = Modifier.isPublic(modifiers) && Modifier.isStatic(modifiers) && Modifier.isFinal(modifiers) && propertiesField.getType() == BlockProperties.class;
			if (!validField) {
				throw new IllegalArgumentException("Block class must declare public static final " + "BlockProperties PROPERTIES");
			}
			if (propertiesField.get(null) != block.getProperties()) {
				throw new IllegalArgumentException("getProperties() must return the class PROPERTIES field");
			}
			if (blockClass.getMethod("getProperties").getDeclaringClass() != blockClass) {
				throw new IllegalArgumentException("Block class must override getProperties()");
			}
		} catch (NoSuchFieldException exception) {
			throw new IllegalArgumentException("Block class must declare public static final " + "BlockProperties PROPERTIES", exception);
		} catch (ReflectiveOperationException exception) {
			throw new IllegalArgumentException("Cannot inspect block properties", exception);
		}
	}
