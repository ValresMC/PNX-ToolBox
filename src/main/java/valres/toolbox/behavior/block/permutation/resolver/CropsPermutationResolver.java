...
final public class CropsPermutationResolver extends PermutationsResolver {
    @Override public boolean supports(@NonNull BlockBuilder builder) {
        return builder.getBlock() instanceof BlockCrops
            || builder.getBlock() instanceof BlockNetherWart;
    }
    ...
    @Override public void resolve(@NonNull BlockBuilder builder) {
        ...
        if (!builder.getProperties().containProperty(growthProperty)) {
            throw new IllegalStateException(
                "Crop block '" + builder.getIdentifier()
                    + "' must declare the '" + growthProperty.getName()
                    + "' property"
            );
        }
        ...
        for (int growth = 0; growth <= maximumGrowth; growth++) {
            float height = maximumGrowth == 0
                ? 11.2f
                : ((growth + 1f) / maximumGrowth) * 0.7f * 16f;

            BlockPermutation permutation = new BlockPermutation(
                BlockStateQuery.equals(growthProperty.getName(), growth)
            );
            permutation.addComponent(MaterialInstancesComponent.all(
                new MaterialInstance(
                    builder.getName() + "_" + growth,
                    RenderMethod.ALPHA_TEST
                )
            ));
            permutation.addComponent(SelectionBoxComponent.box(
                List.of(-8f, 0f, -8f),
                List.of(16f, height, 16f)
            ));
