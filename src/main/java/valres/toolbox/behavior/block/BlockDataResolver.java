...
public final class BlockDataResolver {
	...
	public static void applyDefault(@NonNull BlockBuilder builder) {
		...
		RenderMethod renderMethod = block.isTransparent() ? RenderMethod.ALPHA_TEST_SINGLE_SIDED : RenderMethod.OPAQUE;

		builder.addComponent(new GeometryComponent());
		builder.addComponent(MaterialInstancesComponent.all(new MaterialInstance(builder.getName(), renderMethod)));
		builder.addComponent(new DisplayNameBlockComponent("tile." + builder.getIdentifier() + ".name"));
		builder.addComponent(new CollisionBoxComponent(block.getCollisionBoxes().length > 0));
		builder.addComponent(new SelectionBoxComponent(true));

		double resistance = block.getResistance();
		builder.addComponent(resistance < 0 ? new DestructibleByExplosionComponent(false) : new DestructibleByExplosionComponent(resistance));

		double hardness = block.getHardness();
		builder.addComponent(hardness < 0 ? new DestructibleByMiningComponent(false) : new DestructibleByMiningComponent(hardness * 3.33334));

		builder.addComponent(new FrictionBlockComponent(block.getFrictionFactor()));
		builder.addComponent(new LightDampeningComponent(block.getLightFilter()));
		builder.addComponent(new LightEmissionComponent(block.getLightLevel()));
		builder.addComponent(new MapColorComponent(toRgbHex(resolveColor(block))));
	...
	private static void applyCreativePlacement(@NonNull BlockBuilder builder) {
		CreativeInventoryInfo info = builder.getBlock().getClass().getAnnotation(CreativeInventoryInfo.class);
		if (info == null) {
		...
		if (!customGroup.isEmpty() && info.group() != CreativeGroup.NONE) {
			throw new IllegalStateException("Block '" + builder.getIdentifier() + "' cannot define both group and customGroup");
		}

		CreativeCategory category = info.hidden() ? CreativeCategory.NONE : info.category();
		String group = customGroup.isEmpty() ? groupName(info.group()) : customGroup;

		builder.setCreativePlacement(category, group).setHiddenInCommands(info.hidden());
	}
	...
	private static @NonNull BlockColor resolveColor(@NonNull Block block) {
		try {
			boolean usesBaseColor = block.getClass().getMethod("getColor").getDeclaringClass() == Block.class;
