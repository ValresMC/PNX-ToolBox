...
abstract public class BlockTrait {
    ...
    public @NonNull CompoundTag toNBT() {
        ...
        for (Object state : Objects.requireNonNull(
            this.enabledStates(),
            "Enabled trait states cannot be null"
        )) {
        ...
        CompoundTag tag = new CompoundTag()
            ...
            .putCompound("enabled_states", enabledStates);

        for (Map.Entry<String, ?> entry : Objects.requireNonNull(
            this.values(),
            "Trait values cannot be null"
        ).entrySet()) {
            String name = Objects.requireNonNull(
                entry.getKey(),
                "Trait value name cannot be null"
            );
            Object value = entry.getValue();
            if (value != null) {
                tag.put(
                    name,
                    value instanceof Tag existing
                        ? existing
                        : ComponentNbtHelper.tag(value)
                );
            }
        }
