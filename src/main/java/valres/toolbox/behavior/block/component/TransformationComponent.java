...
public final class TransformationComponent extends BlockComponent {
	...
	/**
	 * @deprecated PNX network NBT does not support rotation or scale pivots. Use
	 *             {@link #TransformationComponent(List, List, List)} instead.
	 */
	@Deprecated(forRemoval = true) public TransformationComponent(@Nullable List<? extends Number> translation, @Nullable List<? extends Number> rotation, @Nullable List<? extends Number> rotationPivot, @Nullable List<? extends Number> scale, @Nullable List<? extends Number> scalePivot) {
		this(translation, rotation, scale);

		if (rotationPivot != null || scalePivot != null) {
			throw new IllegalArgumentException("rotationPivot and scalePivot are not supported by PNX network NBT");
		}
	}
	...
	@Override public @NonNull Tag toNBT() {
		return new CompoundTag().putInt("RX", quarterTurns(this.rotation, 0)).putInt("RY", quarterTurns(this.rotation, 1)).putInt("RZ", quarterTurns(this.rotation, 2)).putFloat("SX", component(this.scale, 0, 1.0f)).putFloat("SY", component(this.scale, 1, 1.0f)).putFloat("SZ", component(this.scale, 2, 1.0f)).putFloat("TX", component(this.translation, 0, 0.0f)).putFloat("TY", component(this.translation, 1, 0.0f)).putFloat("TZ", component(this.translation, 2, 0.0f));
	}
	...
	private static int quarterTurns(@Nullable List<? extends Number> rotation, int axis) {
		...
		if (!Float.isFinite(degrees) || degrees % 90.0f != 0.0f) {
			throw new IllegalArgumentException("Rotation must be a finite multiple of 90 degrees: " + degrees);
		}
