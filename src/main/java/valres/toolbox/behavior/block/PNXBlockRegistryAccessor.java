...
final public class PNXBlockRegistryAccessor {
    final private static Field keysField;
    final private static Field constructorsField;
    final private static Field stateConstructorsField;
    final private static Field propertiesField;
    final private static Field definitionsField;
    final private static Field definitionsByIdField;
    ...
    static {
        try {
            ...
            stateConstructorsField = accessibleField(
                "CACHE_CONSTRUCTORS_BY_HASH"
            );
            propertiesField = accessibleField("PROPERTIES");
            definitionsField = accessibleField("CUSTOM_BLOCK_DEFINITIONS");
            definitionsByIdField = accessibleField(
                "CUSTOM_BLOCK_DEFINITION_BY_ID"
            );
    ...
    public static void register(
        ...
        Class<? extends Block> blockClass,
        CustomBlockDefinition definition,
        boolean registerDefaultItem
    ) throws RegisterException {
        String identifier = block.getId();
        BlockProperties properties = block.getProperties();

        try {
            Map<String, FastConstructor<? extends Block>> constructors =
                (Map<String, FastConstructor<? extends Block>>) constructorsField.get(
                    null
                );
            ...
            FastMemberLoader loader = loaders.computeIfAbsent(
                blockClass.getClassLoader(),
                FastMemberLoader::new
            );
            FastConstructor<? extends Block> constructor = FastConstructor.create(
                blockClass.getConstructor(BlockState.class),
                loader,
                false
            );
            ...
            if (registerDefaultItem) {
                Registries.ITEM_RUNTIMEID.registerCustomRuntimeItem(
                    new ItemRuntimeIdRegistry.RuntimeEntry(
                        identifier,
                        resolveItemRuntimeId(definition),
                        false
                    )
                );
            }
            ...
            Map<Integer, FastConstructor<? extends Block>> stateConstructors =
                (Map<Integer, FastConstructor<? extends Block>>) stateConstructorsField.get(
                    null
                );
            for (BlockState state : properties.getSpecialValueMap().values()) {
                Registries.BLOCKSTATE.registerInternal(state);
                stateConstructors.putIfAbsent(
            ...
            ((Map<String, BlockProperties>) propertiesField.get(null)).put(
                identifier,
                properties
            );

            Plugin owner = ToolBox.getInstance();
            Map<Plugin, List<CustomBlockDefinition>> definitions =
                (Map<Plugin, List<CustomBlockDefinition>>) definitionsField.get(
                    null
                );
            ...
            ((Map<String, CustomBlockDefinition>) definitionsByIdField.get(null)).put(
                identifier,
                definition
            );

            if (registerDefaultItem
                && Registries.CREATIVE.shouldBeRegisteredBlock(definition.nbt())) {
                ItemBlock item = new ItemBlock(block);
                item.setNetId(null);
                int groupIndex = Registries.CREATIVE.resolveGroupIndexFromBlockDefinition(
                    identifier,
                    definition.nbt()
                );
    ...
    public static int resolveItemRuntimeId(
        CustomBlockDefinition definition
    ) {
        return 255 - definition.getRuntimeId();
    }

    private static Field accessibleField(
        String name
