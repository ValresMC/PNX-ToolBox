...
public final class CreativeInventoryManager {
	private static final CreativeInventoryManager INSTANCE = new CreativeInventoryManager();
	private static final String SMITHING_TEMPLATES_GROUP = "itemGroup.name.smithing_templates";
	...
	public void add(@NonNull Item item, @NonNull CreativeCategory category, @NonNull CreativeGroup group) {
		this.add(item, category, group == CreativeGroup.NONE ? "" : group.getGroupName());
	}

	public void add(@NonNull Item item, @NonNull CreativeCategory category, @NonNull String groupName) {
	...
	public void applyDefinition(@NonNull DataDrivenItemBuilder builder) {
		...
		builder.addProperty(new CreativeCategoryProperty(placement.hidden() ? CreativeCategory.NONE : placement.category()));
	...
	CreativePlacement resolve(@NonNull Item item) {
		...
		if (info != null) {
			...
			if (!customGroup.isEmpty() && info.group() != CreativeGroup.NONE) {
				throw new IllegalStateException("Item '" + item.getId() + "' cannot define both group and customGroup");
			}

			String groupName = customGroup.isEmpty() ? this.groupName(info.group()) : customGroup;
			boolean hidden = info.hidden() || info.category() == CreativeCategory.NONE || info.category() == CreativeCategory.ITEM_COMMAND_ONLY;
	...
	private CreativePlacement detectPlacement(@NonNull Item item) {
		...
		if (item instanceof SpawnEggPickable || item.hasTag(ItemTags.SPAWN_EGG) || itemPath.endsWith("_spawn_egg")) {
		...
		if (itemPath.endsWith("_smithing_template")) {
			return new CreativePlacement(CreativeCategory.ITEMS, SMITHING_TEMPLATES_GROUP, false);
		}
		...
		if (block instanceof BlockCrops || itemPath.endsWith("_crop") || itemPath.endsWith("_crops")) {
			return this.placement(CreativeCategory.NATURE, CreativeGroup.CROP);
		}
		if (!block.isAir()) {
			return new CreativePlacement(CreativeCategory.CONSTRUCTION, "", false);
		}
	...
	private boolean isUngroupedEquipment(@NonNull Item item) {
		return item.isTool() || item.isShears() || item.isBow() || item.isCrossbow() || item.isSpear() || item.isTrident() || item.isMace() || item.isShield();
	}
	...
	private boolean isSeed(@NonNull Item item, @NonNull String itemPath) {
		...
		return itemPath.endsWith("_seed") || itemPath.endsWith("_seeds") || className.endsWith("seed") || className.endsWith("seeds");
	}

	private @NonNull CreativePlacement placement(@NonNull CreativeCategory category, @NonNull CreativeGroup group) {
		return new CreativePlacement(category, this.groupName(group), false);
	}
	...
	private int resolveGroupIndex(@NonNull String identifier, @NonNull CreativeCategory category, @NonNull String groupName) {
		if (groupName.isBlank()) {
			return CreativeItemRegistry.getLastGroupIndexFrom(category.name());
		}

		CompoundTag properties = new CompoundTag().putInt("creative_category", category.getId()).putString("creative_group", groupName);
		CompoundTag definition = new CompoundTag().putCompound("components", new CompoundTag().putCompound("item_properties", properties));
	...
	record CreativePlacement(CreativeCategory category, String groupName, boolean hidden) {
		private static final CreativePlacement DEFAULT = new CreativePlacement(CreativeCategory.ITEMS, "", false);
	}
}
