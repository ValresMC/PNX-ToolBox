...
public final class Rcon implements AutoCloseable {
	private static final int MAX_COMMANDS_PER_TICK = 64;

	private final Server server;
	private final RconCommandExecutor commandExecutor;
	private final RconInterface rconInterface;
	private final ConcurrentLinkedQueue<PendingCommand> pendingCommands = new ConcurrentLinkedQueue<>();
	private final AtomicBoolean closed = new AtomicBoolean();

	public Rcon(@NonNull Server server, @NonNull String password, @NonNull String address, int port) {
		this(server, server.getLogger(), RconSettings.defaults(password, port, address), null);
	}

	public Rcon(@NonNull Server server, @NonNull Logger logger, @NonNull RconSettings settings) {
		this(server, logger, settings, null);
	}

	public Rcon(@NonNull Server server, @NonNull Logger logger, @NonNull RconSettings settings, RconCommandExecutor commandExecutor) {
	...
	private String waitForPrimaryThread(String command) throws Exception {
		if (this.closed.get()) {
			throw new RconException("RCON is shutting down");
		}
		...
		try {
			...
			throw new RconException("RCON command execution failed", cause);
		}
	}
	...
	@Override public void close() {
		...
		RconException shutdown = new RconException("RCON was stopped before the command could run");
