...
public record RconPacket(int requestId, int type, @NonNull String payload) {
	public static final int MINIMUM_BODY_SIZE = 10;

	public RconPacket {
		payload = Objects.requireNonNull(payload, "RCON payload cannot be null");

		if (payload.indexOf('\0') >= 0) {
			throw new RconProtocolException("RCON payload cannot contain a null character");
		}
	}
	...
	public static @NonNull RconPacket decode(@NonNull byte[] body) {
		...
		if (body.length < MINIMUM_BODY_SIZE) {
			throw new RconProtocolException("RCON packet body is shorter than 10 bytes");
		}
		if (body[body.length - 2] != 0 || body[body.length - 1] != 0) {
			throw new RconProtocolException("RCON packet is missing its null terminators");
		}
	...
	private static @NonNull String decodeUtf8(@NonNull byte[] payload) {
		try {
			...
			throw new RconProtocolException("RCON payload is not valid UTF-8", exception);
		}
	}
	...
	@Override public @NonNull String toString() {
		return "RconPacket[requestId=%d, type=%d, payloadBytes=%d]".formatted(this.requestId, this.type, this.payload.getBytes(StandardCharsets.UTF_8).length);
	}
}
